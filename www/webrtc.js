...
function Session(our_id, peer_id, closed_callback) {
  ...
  this.onRemoteDescriptionSet = function () {
    ...
    this.peer_connection
      .createAnswer()
      .then(this.onLocalDescription.bind(this))
      .catch(this.setError);
  };
  ...
  this.onServerMessage = function (event) {
    ...
    } else if (msg.type == "endSessionError") {
      this.resetState();
      this.closed_callback(this.our_id);
  ...
  this.websocketServerConnect = function () {
    ...
    if (window.location.protocol.startsWith("file")) {
      ...
      throw new Error(
        "Don't know how to connect to the signalling server with uri" +
          window.location
      );
    }
  ...
  document
    .getElementById("stream-" + this.our_id)
